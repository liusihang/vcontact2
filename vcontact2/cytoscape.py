...
def contigs(network, contigs, clusters, path):
    ...
    with open(fi_ntw,"w") as f:
        ...
        for r,c in combinations(contigs.pos,2):
            if network[r,c]:
                n1 = contigs.loc[r,"name"] 
                n2 = contigs.loc[c,"name"] 
                f.write("\t".join([str(n1),
                                   "contig-contig",
                                   str(n2),
                                   str(network[r,c])]))
        ...
        for r in range(cluster_network.shape[0]):
            for c in range(r):
                if cluster_network[r,c]:
                    f.write("\t".join([clusters.loc[r,"name"],
                                       clusters.loc[c,"name"],
...
def cluster_network_cytoscape(self,fi_ntw=None,fi_clust_info=None,network=None):
        ...
        with open(fi_ntw,"w") as f:
            ...
            for r in range(L.shape[0]):
                for c in range(r):
                    if L[r,c]:
                        n1 = cluster_idx.loc[r,"name"] if r != L.shape[0]-1 else "non-clustered" 
                        n2 = cluster_idx.loc[c,"name"] if c != L.shape[0]-1 else "non-clustered"
                        f.write("\t".join([str(n1),
                                           str(n2),
                                           str(L[r,c])]))
                        f.write("\n")        
...
def export_network(fi_ntw, network, dataframe,
    ...
    with open(fi_ntw,"w") as f:
        ...
        for r,c in combinations(pos,2) :
                if network[r,c]:
                    n1 = dataframe.loc[r,"name"] 
                    n2 = dataframe.loc[c,"name"] 
                    f.write("\t".join([str(n1),
                                       "contig-contig",
                                       str(n2),
                                       str(network[r,c])]))
                    ...
                    if network[pos_c,:][:,pos_c].sum():
                        for n in pos:
                            ...
                            if s and membership[n,c] > 0.1:
                                f.write("\t".join([dataframe.loc[n,"name"],
                                                   "contig-cluster",
                                                   "cluster_{}".format(c),
                                                   str(s)]))
                                f.write("\n")
...
def export_membership(fi_mb, membership, dataframe, pos):
    ...
    with open(fi_mb,"wb") as f:
        ...
        for i,r in enumerate(pos):
                f.write("{0}\t ".format(dataframe.loc[r,"name"]))
